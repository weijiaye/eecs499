...
for i = 1:size(x_rmp,2)
    ...
    if (cnnct(1,i,C)==1)&&(cnnct(2,i,C)==1)
        route_x(n,1:3)=[x_rmp(1) x_rmp(i) x_rmp(2)];
        route_y(n,1:3)=[y_rmp(1) y_rmp(i) y_rmp(2)];
        length(n) = 0;
        for m = 1:2
            length(n) = length(n) + norm([(route_x(n,m+1)-route_x(n,m)) (route_y(n,m+1)-route_y(n,m))]);
        end
        %line(route_x(n,1:3),route_y(n,1:3),'linewidth',2,'color','r');
        n = n + 1;
    end
end
if n <= 200;
    for i = 1:size(x_rmp,2)
        ...
        for j = 1:size(x_rmp,2)
            ...
            if (cnnct(1,i,C)==1)&&(cnnct(j,i,C)==1)&&(cnnct(2,j,C)==1)
                route_x(n,1:4)=[x_rmp(1) x_rmp(i) x_rmp(j) x_rmp(2)];
                route_y(n,1:4)=[y_rmp(1) y_rmp(i) y_rmp(j) y_rmp(2)];
                length(n) = 0;
                for m = 1:3
                    length(n) = length(n) + norm([(route_x(n,m+1)-route_x(n,m)) (route_y(n,m+1)-route_y(n,m))]);
                end
                %line(route_x(n,1:4),route_y(n,1:4),'linewidth',2,'color','y');
            end
        end
    end
end
if n <= 200
    for i = 1:size(x_rmp,2)
        ...
        for j = 1:size(x_rmp,2)
            ...
            for k = 1:size(x_rmp,2)
                if (1 == k)||(2 == k)||(i == k)||(j == k)
                    continue
                end
                if (cnnct(1,i,C)==1)&&(cnnct(j,i,C)==1)&&(cnnct(k,j,C)==1)&&(cnnct(2,k,C)==1)
                    route_x(n,1:5)=[x_rmp(1) x_rmp(i) x_rmp(j) x_rmp(k) x_rmp(2)];
                    route_y(n,1:5)=[y_rmp(1) y_rmp(i) y_rmp(j) y_rmp(k) y_rmp(2)];
                    length(n) = 0;
                    for m = 1:4
                        length(n) = length(n) + norm([(route_x(n,m+1)-route_x(n,m)) (route_y(n,m+1)-route_y(n,m))]);
                    end
                    %line(route_x(n,1:5),route_y(n,1:5),'linewidth',2,'color','y');
                    n = n + 1;
                end
            end
        end
...
for i = 1:size(length,2)
    length_e(i) = length(i);
    route_e(i,2:6) = route_x(i,1:5);
    route_e(i,7:11) = route_y(i,1:5);
    route_e(i,1) = length_e(i);
end
route_ee = sortrows(route_e,1);
for i = 1:route_num
    if route_ee(i,5) == 0
        line(route_ee(i,2:4),route_ee(i,7:9),'linewidth',i,'color','r');
    elseif route_ee(i,6) == 0
        line(route_ee(i,2:5),route_ee(i,7:10),'linewidth',i,'color','r');
    else
        line(route_ee(i,2:6),route_ee(i,7:11),'linewidth',i,'color','r');
    end
end